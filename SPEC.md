...
- BGM 推荐切换仍需要用户确认；播放器的上一首、下一首和单曲结束后自动下一首属于当前播放队列行为，按可播放曲目在列表中的顺序执行。
- 系统 BGM 曲库元数据位于 `frontend/src/bgm/builtInTracks.ts`；若需要 demo 音频，放入 `frontend/public/bgm/` 并通过 `fileRef` 引用。用户上传音频仍只保存在浏览器本地。
